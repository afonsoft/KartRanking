...
(function(jQuery) {
    ...
    jQuery.fn.easySlider = function(options) {
        ...
        var defaults = {
            ...
            speed: 3000,                    // duration of a transition
            ...
            hoverPause: 6500,                 // when hovering, the time between transitions
        ...
        return this.each(function() {
            ...
            var obj = jQuery(this),
          totalSlides = jQuery("li", obj).length,
          ...
          restart = null;
            ...
            if (autogen) {
                jQuery(obj).after('<span id="' + options.prevId + '"><a href=\"javascript:void(0);\">' + options.prevText + '</a></span> ' +
                      '<span id="' + options.nextId + '"><a href=\"javascript:void(0);\">' + options.nextText + '</a></span>');
            }
            // Se tiver um unico item não entrar em loop
            if (totalSlides <= 1)
                loop = false;

            // Important Elements
            var $ul = jQuery("ul", obj),
          $next = jQuery("#" + options.nextId),
          $prev = jQuery("#" + options.prevId);

            // Loop - Duplicate the First Slide onto the end
            if (loop) {
                $ul.append(jQuery("li:first", obj).clone());
                totalSlides += 1;
                lastSlide += 1;
            }
            ...
            if (pauseable && autoplay) {
                jQuery(obj).hover(pauseFunc, restartFunc);
                if (buttonsPause) {
